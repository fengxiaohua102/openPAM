...
if __name__ == '__main__':
    ...
    data_dir = imagingsession_dir + '/results/'
    N_file = 1    # number of files for the imaging session
    ...
    for K in range(N_file):
        f_n = K+1
        PAM_ROI, num_focal_left = read_dataset(imagingsession_dir, f_n)
        ...
        print('FISTA located coarse index and shift is:', np.int32(K_idx), shift_opt_coarse)
        print('FISTA located fine index and shift is:', np.int32(K_idx2), shift_opt_fine)
        
        shift_opt_array[K] = np.int32(shift_opt_fine)
        
        plt.figure()
        plt.plot(Phys_loss_fista) 
        plt.title('Coarse search via objectiv loss')
        
        plt.figure()
        plt.plot(Phys_loss_fista2) 
        plt.title('Fine search via objectiv loss')
